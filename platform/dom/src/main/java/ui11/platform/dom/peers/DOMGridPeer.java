...
public class DOMGridPeer extends DOMLayoutPeerBase {
    ...
    @State private BitSet goneItems;
    ...
    protected List<? extends HTMLElement> children() {
        ...
        for (int i = 0; i < items.size(); i++) {
            ...
            WidgetInstantiation childH = slots.instantiate(key, new DOMWidgetWrapper(item.widget()));
            if (childH.lookupOptional(Gone.class).isPresent())
                goneItems.set(i);
            else
                childrenPeers.add(childH.lookup(DOMElementHolder.class).element());
        }
