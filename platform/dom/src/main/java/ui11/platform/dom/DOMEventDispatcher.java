...
class DOMEventDispatcher {
    ...
    @Nonnull
    private Location locationOfMouseEvent(MouseEvent evt) {
        return new Location(env.clientCoordinateSpace.origin, pointOfMouseEvent(evt));
    }

    @Nonnull
    private static Vec2 pointOfMouseEvent(MouseEvent evt) {
        return new Vec2(evt.getClientX(), evt.getClientY());
    }

    @Nonnull
    private static Vec2 pointOfTouch(JSTouch evt) {
        return new Vec2(evt.getClientX(), evt.getClientY());
    }

    @Nonnull
    private Location locationOfTouch(JSTouch touch) {
