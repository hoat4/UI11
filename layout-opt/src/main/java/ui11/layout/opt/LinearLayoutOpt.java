...
class LinearLayoutOpt extends Widget {

    private final LinearLayout linearLayout;

    // a következő kettő közül pontosan az egyik nemnull
    private final @Nullable PeerRequest<Widget> transformedWidgetRequest;
    private final @Nullable CollapsedLLRequest collapsedLLRequest;

    public LinearLayoutOpt(LinearLayout linearLayout,
                           @Nullable PeerRequest<Widget> transformedWidgetRequest,
                           @Nullable CollapsedLLRequest collapsedLLRequest) {
        this.linearLayout = linearLayout;
        this.transformedWidgetRequest = transformedWidgetRequest;
        this.collapsedLLRequest = collapsedLLRequest;
    }

    @Override
    protected Widget build() {
        // TODO ha collapsedLLRequest == nulll, akkor weightokat nem is kéne lekérdezni
    ...
    private Widget processResults(Map<PeerRequest<?>, ? extends List<?>> results) {
        ...
        if (collapsedLLRequest != null) {
            if (linearLayout.mainAxisAlignment() == LinearLayout.JustifyContent.STRETCH)
                return collapsedLLRequest.createResponse(new CollapsedLL(newLL, newWeights), newLL);
            else
                return newLL;
        } else {
            assert transformedWidgetRequest != null;
